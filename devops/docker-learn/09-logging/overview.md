...
生产环境中处理容器日志（尤其是输出到 stdout/stderr 的日志）的标准做法是构建一个集中的日志管理系统。这个系统通常包含以下几个关键部分：
...
1.  EFK/ELK Stack:
    *   Elasticsearch / OpenSearch: 存储和索引日志，提供强大的搜索能力。
    *   Fluentd / Logstash: 日志收集代理，负责收集、处理和转发日志。Fluentd 更轻量，常用于容器环境。
    *   Kibana: 数据可视化和查询界面。
