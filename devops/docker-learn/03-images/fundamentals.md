...
这里先讲 image 和 container 的关系。重点是“image 是静态模板，container 是运行实例”。
...
- `repository`: image 命名空间，用于区分镜像的来源
- `image-name`: 镜像名
- `tag`: 标签，具体说明镜像的变体，用于选择特定版本或配置
...
- `modsecurity`: 该镜像的主要功能是提供 ModSecurity WAF
- `:nginx-alpine`: 表示这个镜像包含 Nginx Web 服务器，基础操作系统是 Alpine Linux。
