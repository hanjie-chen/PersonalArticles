# Nginx in Modescuirty

在 owasp/modsecurity 这个镜像中，存在 nginx 作为 reverse proxy 的镜像

存在2种，分别是 nginx, nginx-alpine，这里2个都有他自己的 dockerfile
...
# `owasp/modsecurity-crs:nginx-alpine` running log
...
当我们尝试去运行 `owasp/modsecurity-crs:nginx-alpine` image 的时候，可以看到这样子的log
